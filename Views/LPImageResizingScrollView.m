...
{    
    ...
    switch (self.image.imageOrientation)
    {
        ...
        case UIImageOrientationLeftMirrored:
            rectRotationTransform = CGAffineTransformTranslate(CGAffineTransformMakeRotation(M_PI/2), 0, -self.image.size.height);
            break;
        case UIImageOrientationRight:
        case UIImageOrientationRightMirrored:
            rectRotationTransform = CGAffineTransformTranslate(CGAffineTransformMakeRotation(-M_PI/2), -self.image.size.width, 0);
            break;
        case UIImageOrientationDown:
        case UIImageOrientationDownMirrored:
            rectRotationTransform = CGAffineTransformTranslate(CGAffineTransformMakeRotation(-M_PI), -self.image.size.width, -self.image.size.height);
            break;
        default:
            rectRotationTransform = CGAffineTransformIdentity;
    };
    rectRotationTransform = CGAffineTransformScale(rectRotationTransform, self.image.scale, self.image.scale);
...
//    NSLog(@"Info: self.bounds.size.height / effectiveScale %f", self.bounds.size.height / effectiveScale);
    ...
    UIImage *result = [UIImage imageWithCGImage:imageRef scale:1.0 orientation:self.image.imageOrientation];
...
- (void)loadPhotoWithPreZoomPadding:(CGFloat)preZoomPadding
{
    ...
    CGRect imageViewFrame = CGRectMake(0.0f, 0.0f, innerW, innerH);
    self.contentSize = imageViewFrame.size;
    
    self.contentOffset = CGPointMake((innerW - self.frame.size.width)/2, (innerH - self.frame.size.height)/2); // center the view
